...
protected:
  ...
  //======
  eidType *d_small_rowptr;               
  vidType *d_small_colidx;            
  eidType *d_large_rowptr;               
  vidType *d_large_colidx;
  //======
public:
  //=====
  inline __device__ __host__ eidType small_edge_begin(vidType src) { return d_small_rowptr[src]; }
  inline __device__ __host__ eidType small_edge_end(vidType src) { return d_small_rowptr[src+1]; }
  inline __device__ __host__ vidType get_small_EdgeDst(eidType edge) { return d_small_colidx[edge]; }
  
  inline __device__ __host__ eidType large_edge_begin(vidType src) { return d_large_rowptr[src]; }
  inline __device__ __host__ eidType large_edge_end(vidType src) { return d_large_rowptr[src+1]; }
  inline __device__ __host__ vidType get_large_EdgeDst(eidType edge) { return d_large_colidx[edge]; }
  GraphGPU(Graph &g, int divide) : device_id(0), n_gpu(1) { if(divide) init_with_division(g); }
  //=====
  ...
  void init_with_division(Graph &hg) {
    auto m = hg.num_vertices();
    auto nnz = hg.num_edges();
    num_vertices = m;
    num_edges = nnz;
    ...
    auto h_small_rowptr = hg.small_out_rowptr();
    auto h_small_colidx = hg.small_out_colidx();
    auto small_nnz = hg.num_small_edges();
    auto large_nnz = hg.num_large_edges();
    auto h_large_rowptr = hg.large_out_rowptr();
    auto h_large_colidx = hg.large_out_colidx();
    Timer t;
    {
      //std::cout << "Allocating graph memory on GPU" << device_id << "\n";
      CUDA_SAFE_CALL(cudaMalloc((void **)&d_rowptr, (m + 1) * sizeof(eidType)));
      ...
      CUDA_SAFE_CALL(cudaMalloc((void **)&d_small_rowptr, (m + 1) * sizeof(eidType)));
      CUDA_SAFE_CALL(cudaMalloc((void **)&d_small_colidx, small_nnz * sizeof(vidType)));
      CUDA_SAFE_CALL(cudaMalloc((void **)&d_large_rowptr, (m + 1) * sizeof(eidType)));
      CUDA_SAFE_CALL(cudaMalloc((void **)&d_large_colidx, large_nnz * sizeof(vidType)));
      CUDA_SAFE_CALL(cudaDeviceSynchronize());
      t.Start();
      CUDA_SAFE_CALL(cudaMemcpy(d_rowptr, h_rowptr, (m + 1) * sizeof(eidType), cudaMemcpyHostToDevice));
      CUDA_SAFE_CALL(cudaMemcpy(d_colidx, h_colidx, nnz * sizeof(vidType), cudaMemcpyHostToDevice));
      
      CUDA_SAFE_CALL(cudaMemcpy(d_small_rowptr, h_small_rowptr, (m + 1) * sizeof(eidType), cudaMemcpyHostToDevice));
      CUDA_SAFE_CALL(cudaMemcpy(d_small_colidx, h_small_colidx, small_nnz * sizeof(vidType), cudaMemcpyHostToDevice));
      
      CUDA_SAFE_CALL(cudaMemcpy(d_large_rowptr, h_large_rowptr, (m + 1) * sizeof(eidType), cudaMemcpyHostToDevice));
      CUDA_SAFE_CALL(cudaMemcpy(d_large_colidx, h_large_colidx, large_nnz * sizeof(vidType), cudaMemcpyHostToDevice));
  ...
  void copy_edgelist_to_device(std::vector<eidType> lens, std::vector<vidType*> &srcs, std::vector<vidType*> &dsts, int rank) {
    //Timer t;
    //t.Start();
    vidType* src_ptr = srcs[rank];
    vidType* dst_ptr = dsts[rank];
    auto num = lens[rank];
    //std::cout << "src_ptr = " << src_ptr << " dst_ptr = " << dst_ptr << "\n";
    //std::cout << "Allocating edgelist on GPU" << device_id << " size = " << num << "\n";
    CUDA_SAFE_CALL(cudaMalloc((void **)&d_src_list, num * sizeof(vidType)));
